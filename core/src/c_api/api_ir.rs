...
use crate::logic::static_analysis::disasm::{decode_instr_single, create_instr_invalid, ArsitekturDisasm, C_Instruksi};
...
pub unsafe extern "C" fn c_liftInstruksi(
    ...
    if ptr_kode_raw.is_null() {
        set_err_last(ReToolsError::Generic("Code pointer null".to_string()));
    ...
    match result {
        Ok(ptr) => ptr,
        Err(_) => {
            set_err_last(ReToolsError::Generic("Panic di c_liftInstruksi".to_string()));
            ptr_json_kosong
        }
    }
}
...
pub unsafe extern "C" fn c_calcOptimasi(
    ...
    match result {
        Ok(ptr) => ptr,
        Err(_) => {
            set_err_last(ReToolsError::Generic("Panic di c_calcOptimasi".to_string()));
            ptr_json_error
        }
    }
}