...
use crate::logic::static_analysis::cfg::generateCfgGraph;
...
#[allow(non_snake_case)]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn c_scanCryptoConstants_json(file_path_c: *const c_char) -> *mut c_char {
	c_load_binary_and_serialize(file_path_c, |binary| scan_crypto_constants_internal(binary))
}
...
pub unsafe extern "C" fn c_buatCFG(filename_c: *const c_char) -> *mut c_char {
	...
	let dot_result = match binary_result {
		Ok(binary) => match generateCfgGraph(&binary) {
			Ok(dot) => dot,
			Err(e) => {
...
#[allow(non_snake_case)]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn c_decompileFunction(
	file_path_c: *const c_char,
	function_address: u64,
) -> *mut c_char {
	let error_str = CString::new("/* Decompilation Error */").unwrap().into_raw();
	let path_str = match CStr::from_ptr(file_path_c).to_str() {
		Ok(s) => s,
		Err(e) => {
			set_last_error(e.into());
	...
	match decompile_function_internal(&binary, function_address) {
		Ok(pseudocode) => CString::new(pseudocode).unwrap_or_default().into_raw(),
		Err(e) => {
			set_last_error(e);
			error_str
		}
