...
where
	F: FnOnce(&Binary) -> Result<T, ReToolsError> + std::panic::UnwindSafe,
	T: serde::Serialize,
{
	let ptr_json_error = CString::new("[]").unwrap().into_raw();
	if ptr_jalur_raw.is_null() {
		set_err_last(ReToolsError::Generic("Pointer path null".to_string()));
		return ptr_json_error;
	}
	let result = std::panic::catch_unwind(|| {
		let str_jalur_sumber = match unsafe { CStr::from_ptr(ptr_jalur_raw) }.to_str() {
			Ok(s) => s,
			Err(e) => {
				set_err_last(e.into());
				return ptr_json_error;
	...
	match result {
		Ok(ptr) => ptr,
		Err(_) => {
			set_err_last(ReToolsError::Generic("Panic terjadi di Rust core".to_string()));
			ptr_json_error
		}
	}
}
...
pub unsafe extern "C" fn c_scanPola_rs(
	...
	let result = std::panic::catch_unwind(|| {
		...
		let str_regex = match unsafe { CStr::from_ptr(ptr_pola_regex) }.to_str() {
			Ok(s) => s,
			Err(e) => {
				set_err_last(e.into());
				return -1;
		...
		let slice_luaran = unsafe { std::slice::from_raw_parts_mut(ptr_buf_luaran, sz_buf_maks as usize) };
	...
	match result {
		Ok(val) => val,
		Err(_) => {
			set_err_last(ReToolsError::Generic("Panic terjadi di c_scanPola_rs".to_string()));
			-1
		}
	}
}
...
pub unsafe extern "C" fn c_scanYara_rs(
	...
	let ptr_json_error = CString::new("[]").unwrap().into_raw();
	if ptr_jalur_raw.is_null() || ptr_aturan_yara.is_null() {
		set_err_last(ReToolsError::Generic("Pointer null detected".to_string()));
		return ptr_json_error;
	}
	unsafe {
		c_load_binary_and_serialize(ptr_jalur_raw, |obj_biner| {
...
pub unsafe extern "C" fn c_scanLib(
	...
	let ptr_json_error = CString::new("[]").unwrap().into_raw();
	if ptr_json_sig.is_null() {
		set_err_last(ReToolsError::Generic("Pointer sig null".to_string()));
		return ptr_json_error;
	}
	unsafe {
		c_load_binary_and_serialize(ptr_jalur_raw, |obj_biner| {
...
pub unsafe extern "C" fn c_calcDiffBiner_json(
	...
	let ptr_json_error = CString::new("[]").unwrap().into_raw();
	if ptr_path1_raw.is_null() || ptr_path2_raw.is_null() {
		set_err_last(ReToolsError::Generic("Pointer path null".to_string()));
		return ptr_json_error;
	}
	let result = std::panic::catch_unwind(|| {
		let str_path1 = match unsafe { CStr::from_ptr(ptr_path1_raw) }.to_str() {
			Ok(s) => s,
			Err(e) => {
				set_err_last(e.into());
				return ptr_json_error;
		...
		let str_path2 = match unsafe { CStr::from_ptr(ptr_path2_raw) }.to_str() {
			Ok(s) => s,
			Err(e) => {
				set_err_last(e.into());
				return ptr_json_error;
	...
	match result {
		Ok(ptr) => ptr,
		Err(_) => {
			set_err_last(ReToolsError::Generic("Panic di c_calcDiffBiner_json".to_string()));
			ptr_json_error
		}
	}
}
...
pub unsafe extern "C" fn c_createCFG(ptr_jalur_raw: *const c_char) -> *mut c_char {
	...
	let ptr_error = CString::new(str_error_dot).unwrap().into_raw();
	if ptr_jalur_raw.is_null() {
		set_err_last(ReToolsError::Generic("Pointer path null".to_string()));
		return ptr_error;
	}
	...
	match result {
		Ok(ptr) => ptr,
		Err(_) => {
			set_err_last(ReToolsError::Generic("Panic di c_createCFG".to_string()));
			ptr_error
		}
	}
}
...
pub unsafe extern "C" fn c_createPseudocode(
	...
	match result {
		Ok(ptr) => ptr,
		Err(_) => {
			set_err_last(ReToolsError::Generic("Panic di c_createPseudocode".to_string()));
			ptr_error_str
		}
	}
}
...
pub unsafe extern "C" fn c_readBytes_hex(
	...
	let result = std::panic::catch_unwind(|| {
		...
		match view_bytes_hex(str_jalur_sumber, off_posisi as u64, len_baca as usize) {
			Ok(str_hex) => {
				...
				let slice_luaran = unsafe { slice::from_raw_parts_mut(ptr_buf_hex, sz_buf_maks as usize) };
	...
	match result {
		Ok(val) => val,
		Err(_) => {
			set_err_last(ReToolsError::Generic("Panic di c_readBytes_hex".to_string()));
			-1
		}
	}
}
...
pub unsafe extern "C" fn c_writeBytes(
	...
	let result = std::panic::catch_unwind(|| {
		...
		let slice_data = unsafe { slice::from_raw_parts(ptr_data_baru, len_data as usize) };
	...
	match result {
		Ok(val) => val,
		Err(_) => {
			set_err_last(ReToolsError::Generic("Panic di c_writeBytes".to_string()));
			0
		}
	}
}
...
pub unsafe extern "C" fn c_scanPolaBytes_json(
	...
	let result = std::panic::catch_unwind(|| {
		...
		let slice_pola = unsafe { slice::from_raw_parts(ptr_pola, len_pola as usize) };
	...
	match result {
		Ok(ptr) => ptr,
		Err(_) => {
			set_err_last(ReToolsError::Generic("Panic di c_scanPolaBytes_json".to_string()));
			ptr_json_error
		}
	}
}
